...
function getDueCountdown(due_date) {
  if (!due_date) return null
  const now = new Date()
  ...
  if (diffMs < 0) {
    ...
    if (diffHours > 0) return `Overdue by ${diffHours} hour${diffHours !== 1 ? 's' : ''}`
    return 'Overdue'
  }
...
function TaskCard({ task, onToggle, onEdit, onDelete }) {
  ...
  const countdown = getDueCountdown(task.due_date)
...
function App() {
  ...
  const [newTask, setNewTask] = useState({ title: '', description: '', priority: 'medium', category: null, due_date: null })
  ...
  const fetchOverdueCount = async () => {
    try {
      const res = await fetch(`${API_BASE}/stats/overdue`)
      if (res.ok) {
  ...
  const createTask = async (e) => {
    ...
    try {
      ...
      setNewTask({ title: '', description: '', priority: 'medium', category: null, due_date: null })
      await fetchTasks()
    } catch (err) {
      setError(err.message)
    } finally {
  ...
  const processedTasks = tasks
    ...
    .filter((t) => !filterOverdue || isOverdue(t))
    .sort((a, b) => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-blue-600 text-white shadow-lg">
        <div className="max-w-3xl mx-auto px-4 py-5 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Test test test</h1>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <form onSubmit={createTask} className="space-y-3">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <input
                type="datetime-local"
                value={newTask.due_date ? toDatetimeLocal(newTask.due_date) : ''}
                onChange={(e) => setNewTask({ ...newTask, due_date: e.target.value ? new Date(e.target.value).toISOString() : null })}
                className="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition bg-white"
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => {
                ...
                if (!sortByPriority) setSortByDueDate(false)
              }}
            ...
            <button
              onClick={() => {
                setSortByDueDate((v) => !v)
                if (!sortByDueDate) setSortByPriority(false)
              }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all duration-150 ${
                sortByDueDate
                  ? 'bg-blue-50 border-blue-300 text-blue-700'
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <form onSubmit={saveEdit} className="space-y-3">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <input
                  type="datetime-local"
                  value={editingTask.due_date ? toDatetimeLocal(editingTask.due_date) : ''}
                  onChange={(e) => setEditingTask({ ...editingTask, due_date: e.target.value ? new Date(e.target.value).toISOString() : null })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition bg-white"
                />
              </div>
